...
object JdbcUtils extends Logging {
  ...
  def getInsertStatement(
    ...
    val updateholders = columns.split(',').map(x => x + "=?").mkString(",")
    if (write_mode != null && write_mode == "upsert") {
      s"INSERT INTO $table ($columns) VALUES ($placeholders) ON DUPLICATE KEY UPDATE $updateholders"
    } else {
  ...
  private type JDBCValueUpsertSetter = (PreparedStatement, Row, Int, Int) => Unit

  private def makeUpsertSetter(
      conn: Connection,
      dialect: JdbcDialect,
      dataType: DataType): JDBCValueUpsertSetter = dataType match {
    case IntegerType =>
  ...
  def savePartition(
      ...
      dialect: JdbcDialect,
      isolationLevel: Int,
      write_mode: String = ""): Iterator[Byte] = {
    ...
    try {
      ...
      val upsert_field = rddSchema.fields ++ rddSchema.fields
      val upsert_setters = upsert_field.map(f => makeUpsertSetter(conn, dialect, f.dataType))
          ...
          while (i < numFields) {
            if (write_mode != null && write_mode == "upsert") {
              if (row.isNullAt(i)) {
                ...
                upsert_setters(i).apply(stmt, row, i, i)
                upsert_setters(i + numFields).apply(stmt, row, i + numFields, i)
              }
            } else {
              if (row.isNullAt(i)) {
                stmt.setNull(i + 1, nullTypes(i))
  ...
  def saveTable(
    ...
    val prop = options.asProperties
    val write_mode = prop.getProperty("write_mode")
    val insertStmt = getInsertStatement(table, rddSchema, tableSchema, isCaseSensitive, dialect, write_mode)
    ...
    repartitionedDF.rdd.foreachPartition(iterator => savePartition(
      getConnection, table, iterator, rddSchema, insertStmt, batchSize, dialect, isolationLevel, write_mode)
    )
  }
